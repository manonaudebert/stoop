"""Normalize HPD violations CSV → data/clean/hpd_violations.parquet.

- Rename columns per HPD_COLUMN_MAP
- Parse date columns
- Drop rows with no violation_id
- Write in chunks to avoid OOM on the ~5 GB source file
"""
import os
...
from config import HPD_COLUMN_MAP, HPD_DB_COLUMNS

RAW_DIR   = os.path.join(os.path.dirname(__file__), '..', 'data', 'raw')
CLEAN_DIR = os.path.join(os.path.dirname(__file__), '..', 'data', 'clean')

DATE_COLS  = ("inspection_date", "approved_date", "certified_date",
              "nov_issued_date", "current_status_date")
CHUNK_SIZE = 200_000


def _clean_chunk(df: pd.DataFrame) -> pd.DataFrame:
    df = df.rename(columns=HPD_COLUMN_MAP)
...
def _find_raw_file() -> str:
    matches = [
        ...
        if f.startswith("Housing_Maintenance_Code_Violations") and f.endswith(".csv")
    ]
    if not matches:
        raise FileNotFoundError(
            "No Housing_Maintenance_Code_Violations*.csv found in data/raw/. "
            "Download it from https://data.cityofnewyork.us/api/views/wvxf-dwi5/rows.csv?accessType=DOWNLOAD"
        )
...
if __name__ == "__main__":
    ...
    out_path = os.path.join(CLEAN_DIR, "hpd_violations.parquet")
