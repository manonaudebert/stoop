-- Add recent_complaint_count, prior_complaint_count, trend_direction
-- to hpd_complaints_building_summary so the HPD complaints leaderboard
-- can rank buildings by activity in the last 2 years — matching the DOB approach.
--
-- Also adds an index on recent_complaint_count DESC for leaderboard perf.
-- Full recompute; takes a few minutes on large datasets.
