-- Add normalized_serious_rate_percentile to building_summary
--
-- Extends the existing normalized_percentile approach to Priority A+B complaints
-- specifically. Divides serious_rate (Priority A+B per year, 10yr window) by
-- building volume (footprint × height proxy) before ranking within NTA peers.
-- Buildings without footprint/height data get NULL (same as normalized_percentile).
--
-- Also drops/recreates nta_stats since it JOINs on building_summary.
...
-- Recreate nta_stats (dropped above via CASCADE)
...
FROM buildings b
JOIN building_summary bs ON b.bin = bs.bin
