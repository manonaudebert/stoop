-- Align DOB building_summary with the HPD 10-year methodology.
--
-- Two changes to the base CTE:
--
-- 1. serious_rate (Severity card / serious_rate_percentile)
--    Was: all-time Priority A+B count ÷ total years on record
--    Now: Priority A+B count in the last 10 years ÷ min(years on record, 10)
--
-- 2. weighted_complaint_sum (Rank card / score_numeric / neighborhood_percentile)
--    Was: decay 2yr=1×, ≤5yr=0.5×, older=0.25× (no cutoff — all history leaks in)
--    Now: decay 2yr=1×, ≤5yr=0.5×, ≤10yr=0.25×, older=0 (hard 10-year cutoff)
--
-- All-time aggregates (total_complaints, priority_ab_complaints, etc.) are
-- unchanged — they reflect the full complaint history.
...
base AS (
    SELECT
        ...
        -- Weighted sum: hard 10-year cutoff, matching HPD methodology.
        COALESCE(SUM(
